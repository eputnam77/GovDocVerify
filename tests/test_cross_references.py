...
class TestCrossReferenceChecks:
    ...
    def test_nested_references(self):
        ...
        assert result["has_errors"]
        assert any(
            "Reference to non-existent section 25.1309" in e["message"] for e in result["errors"]
        )
        # Expect style/inconsistent warnings
        assert any(
            "Inconsistent reference format" in w["message"]
            or "inconsistent" in w["message"].lower()
    ...
    def test_aviation_references(self):
        ...
        content = ["See AC 25-7D", "Refer to FAA Order 8900.1", "Under TSO-C23d"]
        result = self.structure_checks.check(content)
    ...
    def test_regulatory_references(self):
        ...
        content = ["See 14 CFR part 25", "Refer to 49 U.S.C. 44701", "Under 14 CFR § 25.1309"]
        result = self.structure_checks.check(content)
    ...
    def test_reference_existence(self):
        ...
        assert result["has_errors"]
        assert any(
            "Reference to non-existent section 1.2" in e["message"] for e in result["errors"]
        )
    ...
    def test_reference_punctuation(self):
        ...
        assert result["has_errors"]
        assert any(
            "incorrect punctuation" in str(warning).lower() for warning in result["warnings"]
        )
    ...
    def test_reference_capitalization(self):
        ...
        assert result["has_errors"]
        assert any(
            "incorrect capitalization" in str(warning).lower() for warning in result["warnings"]
        )
