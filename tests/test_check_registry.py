...
def test_check_registration():
    ...
    @CheckRegistry.register("test_second")
...
def test_check_discovery():
    ...
    for category, checks in discovered.items():
        ...
        for check in checks:
            ...
            assert check.startswith("check_") or check.startswith(
                "_check_"
...
def test_validation():
    ...
    assert (
        "test.test_check" in validation_results["extra_checks"]
    ...
    for category in discovered:
        if category != "test":
            logger.debug(f"Checking category: {category}")
            assert (
                category in validation_results["missing_categories"]
