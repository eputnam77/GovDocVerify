...
class MockGradioUI:
    def __init__(self):
        ...
        # If you instantiate TerminologyChecks or ReadabilityChecks here, pass self.terminology_manager
        ...
        logging.debug(f"MockGradioUI initialized with TerminologyManager: {self.terminology_manager}")

    def process_document(self, file_path: str, doc_type: str, visibility_settings: VisibilitySettings = None) -> dict:
        if not self.checker:
    ...
    def _get_issue_category(self, issue: dict) -> str:
        ...
        if 'readability' in issue.get('message', '').lower():
            return 'readability'
        elif 'paragraph' in issue.get('message', '').lower() or 'sentence' in issue.get('message', '').lower():
...
class TestGradioUI:
    ...
    def setup(self):
        ...
        import logging
        logging.debug(f"TestGradioUI setup: TerminologyManager: {self.terminology_manager}, MockGradioUI: {self.ui}")
