...
from documentcheckertool.models import DocumentCheckResult, Severity  # Add this import
import logging
import tempfile
import os
import re
                        ...
                        def process_and_format(file_obj, doc_type_value, template_type_value):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    for issue in results_data.issues:
                                        # Log the raw issue for debugging
                                        ...
                                        logger.debug(f"Processed severity: {severity}")
                                    ...
                                    html_parts.append(f"""
                                        <div class="max-w-4xl mx-auto p-4 bg-white rounded-lg shadow-sm mb-6">
                                            <h1 class="text-2xl font-bold mb-4">Document Check Results</h1>
                                            <div class="text-lg text-amber-600">
                                                Found {total_issues} total issues that need attention
                                            </div>
                                        </div>
                                    """)

                                    # Add sections for each severity level that has issues
                                    for severity in ['HIGH', 'MEDIUM', 'LOW']:
                                        ...
                                        html_parts.append(f"""
                                            <div class="bg-white rounded-lg shadow-sm mb-6 overflow-hidden">
                                                <div class="bg-gray-50 px-6 py-4 border-b">
                                                    <h2 class="text-lg font-semibold" style="color: {color}">
